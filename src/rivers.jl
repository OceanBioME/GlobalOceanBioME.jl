...
function river_nutrients(grid; tracers = (:DIN, :DON, :DOC), scalefactor = (10^6/14, 10^6/14, 10^6/12) ./ (365*24*24) .* 1000)
    ...
    forcings = NamedTuple{tracers}(map(n -> CenterField(grid), 1:length(tracers)))
    ...
    CUDA.@allowscalar for n in 1:size(export_df, 1)
        ...
        i, j = Tuple(eachindex(λ)[list_index])
